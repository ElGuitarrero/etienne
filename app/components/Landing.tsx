      ...
      >
        ...
        <motion.div className="w-8/10 flex justify-end" {...animationProps}>
          ...
          <ul className="w-full md:w-1/2 gap-1 text-white lg:text-black flex flex-col items-center font-[sourcesans] font-medium">
            {links.map((item) => (
              <li key={item.nombre}><a href={item.link} className="text-2xl md:text-3xl hover:text-[rgb(240,40,15)]">{item.nombre}</a></li>
            ))}
          </ul>
        </motion.div>
      </div>
      ...
      {showNavbar && (
        ...
        >
          <ul className="flex justify-around p-4">
            {links.map((item) => (
              <li key={item.nombre}><a href={item.link} className="text-xl hover:text-gray-700">{item.nombre}</a></li>
            ))}
          </ul>
        </motion.nav>
      )}
